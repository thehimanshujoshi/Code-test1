...
def get_market_context(ticker, target_dt):
    """Determines market timezone and official opening time based on asset class."""
    if ticker in ["^GSPC", "^DJI", "^IXIC"] or (not ticker.endswith(".NS") and "-USD" not in ticker):
...
TOP_100_INDIA = [
    ("NIFTY 50", "^NSEI"), ("BANK NIFTY", "^NSEBANK"), ("RELIANCE", "RELIANCE.NS"), ("HDFC BANK", "HDFCBANK.NS"), 
    ("TCS", "TCS.NS"), ("INFOSYS", "INFY.NS"), ("ICICI BANK", "ICICIBANK.NS"), ("STATE BANK OF INDIA", "SBIN.NS")
]
GLOBAL_INDICES = [("S&P 500 (USA)", "^GSPC"), ("DOW JONES (USA)", "^DJI"), ("NASDAQ (USA)", "^IXIC")]
CRYPTO_ASSETS = [("Bitcoin", "BTC-USD"), ("Ethereum", "ETH-USD"), ("Solana", "SOL-USD")]
...
with st.spinner(f"Compiling High-Precision Mathematical Targets for {display_name}..."):
    try:
        ...
        st.markdown(f"**⏰ Exchange Local Time:** `{now_local.strftime('%I:%M %p %Z')}` | **🇮🇳 IST Time:** `{now_ist.strftime('%I:%M %p IST')}`")

        target_date_str = target_dt.strftime("%Y-%m-%d")
        # For crypto, expand range to avoid empty weekend frames
        ...
        # Dedicated 15m dataset for strict closing logic
        ...
        if ic > pd_vwap and ic > ie9 and irsi > 55: btst_rec = "🟢 BTST (Buy Call) - Sustaining above VWAP"
        elif ic < pd_vwap and ic < ie9 and irsi < 45: btst_rec = "🔴 STBT (Buy Put) - Bleeding below VWAP"
        ...
        def est_time(target_pts):
            ...
            return (mkt_open_dt + timedelta(minutes=candles * mins_per_candle)).strftime("%I:%M %p")

        # ---------------------------------------------------------
        # SECTION 1: TRADING SIGNALS & TECHNICALS
        # ---------------------------------------------------------
        st.subheader("⚙️ Technical & Price Action Blueprint")
        tech_df = pd.DataFrame({"Metric": ["Used Anchor Price", "Gap Analysis", "Current Intraday Trend (15m Close)", "15m RSI", "BTST / STBT Verdict"], "Value": [f"{OPEN_PRICE:,.2f} ({anchor_desc})", gap_status, day_trend, f"{irsi:.2f}", btst_rec]})
        st.table(tech_df)
        ...
        st.warning("Note: While Daily (Macro) backtesting runs up to 20 years, Intraday (Micro) timeframes are limited to 60 days by Yahoo Finance's free API limits.")
        
        with st.expander("Run Historical Backtest (EOD Daily Data)", expanded=False):
            backtest_years = st.slider("Select Years to Backtest", 1, 20, 5)
            if st.button("🚀 Run Automated Backtest", type="secondary"):
                with st.spinner(f"Simulating daily Gann targets for {display_name} over the last {backtest_years} years..."):
                    bt_data = fetch_market_data(ticker_symbol, datetime.now().strftime("%Y-%m-%d"), f"{backtest_years}y", "1d")
                    
                    if not bt_data.empty and len(bt_data) > 10:
                        hits_long = 0
                        hits_short = 0
                        total_days = len(bt_data) - 10
                        ...
                        for i in range(10, len(bt_data)):
                            ...
                            if d_high >= t1_b_bt: hits_long += 1
                            if d_low <= t1_s_bt: hits_short += 1
                            
                        st.success("Historical Backtest Complete!")
                        bc1, bc2, bc3 = st.columns(3)
                        bc1.metric("Total Trading Days Analyzed", total_days)
                        bc2.metric("Long Target 1 Hit Rate", f"{(hits_long/total_days)*100:.1f}%")
                        bc3.metric("Short Target 1 Hit Rate", f"{(hits_short/total_days)*100:.1f}%")
                    else:
                        st.error("Not enough historical data available from Yahoo Finance for this asset.")
