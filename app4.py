...
def get_market_context(ticker, target_dt):
    if ticker in ["^GSPC", "^DJI", "^IXIC"] or (not ticker.endswith(".NS") and "-USD" not in ticker and "=" not in ticker):
...
if asset_type == "Global Index":
    ...
    custom_input = st.sidebar.text_input("Enter exact Yahoo Finance Ticker (e.g. AAPL, GC=F, EURUSD=X)", "AAPL").strip()
